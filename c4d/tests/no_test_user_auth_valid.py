...
def test_password_minimum_length():
    ...
    with pytest.raises(ValidationError):
        # Trigger full_clean() to run model validations.
        user.full_clean()
...
def test_password_allows_64_chars():
    ...
    # create_user may call full_clean internally or you can call it explicitly.
...
def test_password_allows_unicode():
    ...
    user = User.objects.create_user(
        username="testuser5", email="test5@example.com", password=password
    )
    user.full_clean()
    assert user.check_password(password)
...
def test_user_can_change_password():
    ...
    user = User.objects.create_user(
        username="testuser6", email="test6@example.com", password=original_password
    )
    user.full_clean()
...
def test_password_change_requires_current_and_new_password():
    ...
    user = User.objects.create_user(
        username="testuser7", email="test7@example.com", password=original_password
    )
    user.full_clean()

    # Simulated password change helper
...
def test_breached_password_rejected(monkeypatch):
    ...
    user = User(username="testuser8", email="test8@example.com")
    user.set_password(breached_password)
    with pytest.raises(ValidationError):
        user.full_clean()  # Expecting a validation error because the password is breached.
